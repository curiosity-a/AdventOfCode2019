...
(let (
    ...
    (execute-intcode
        ...
        (lambda ()
            (cond
                (result nil) ; result obtained, yield
            ...
            (if (= 0 output)
                ...
                (progn
                    (when (not (gethash (cons nextx nexty) map))
                        (setf (gethash (cons nextx nexty) map) (1+ (gethash (cons x y) map)))
                        (push (cons x y) path)
                    )
                    (setq x nextx y nexty)
                    (when (= 2 output) (setq result t))
                )
            )
        )
    )
    (print (length path))
    ;(print (gethash (cons x y) map)) ; the same as above
)
