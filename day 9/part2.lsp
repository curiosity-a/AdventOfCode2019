...
(defun execute-intcode (memory input &optional continue-information)
    (let (
        ...
        (loop (multiple-value-bind (arg-mode opcode) (floor (aref memory index) 100)
            ...
            (cond
                ...
                ((= opcode 3)
                    (when (null next-input) ; yield when no input
...
(with-open-file (input-stream "input")
    (let* (
            ...
            (memory (make-array (list (length input)) :initial-contents input :adjustable t))
        )
        (mapcar #'print (reverse
            (execute-intcode memory '(2))
        ))
    )
)
