...
(defun has-precisely-duplicate-digit-p (password)
    (loop for i from 0 upto (- (length password) 2) do
        (when (and
                (char= (aref password i) (aref password (1+ i)))
                (or (= i 0) (char/= (aref password i) (aref password (1- i))))
                (or (= i (- (length password) 2)) (char/= (aref password i) (aref password (+ 2 i))))
            )
            (return t)
        )
    )
...
(with-open-stream (input-stream (open "input"))
    (let* (
        ...
        (loop until (string>= start end) do
            (when (has-precisely-duplicate-digit-p start)
                (incf count)
            )
            (next-password start)
        )
